...
```
condor_submit data_prep_LGATr.sub
```
You may want to adapt the resource requests to the size of your data and the LGATr configuration.
...
Training parameters (e.g. learning rate, batch size, number of epochs) can be configured in `run_LGATr.py`. To start training on a system with HTCondor, use the provided submission file:
```
condor_submit train_LGATr.sub
```
You may want to adapt the resource requests to the size of your data and the LGATr configuration.
